        ...
        with self.get_connection() as conn:
            ...
            if cursor.fetchone()["count"] == 0:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, ("maryam", admin_pw_hash, "Maryam Fahim", "maryam.fahim@carrental.local"))
                logger.info("Owner Admin created: username='maryam'")

            # Check Customers
            # Upgrade an old installation without overwriting a user-customized admin.
            cursor.execute("SELECT id, full_name, email FROM admins ORDER BY id LIMIT 1")
            existing_admin = cursor.fetchone()
            if existing_admin and existing_admin["full_name"] == "System Administrator":
                cursor.execute(
                    "UPDATE admins SET username = ?, full_name = ?, email = ? WHERE id = ?",
                    ("maryam", "Maryam Fahim", "maryam.fahim@carrental.local", existing_admin["id"])
                )
            ...
            if cursor.fetchone()["count"] == 0:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, ("Customer Demo 01", "35201-1234567-1", "0300-1234567", "customer01@demo.local", "Demo Address 01", "customer_demo_01", c1_hash))
                
                cursor.execute("""
                    INSERT INTO customers (full_name, cnic, phone, email, address, username, password)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, ("Customer Demo 02", "35202-9876543-2", "0321-7654321", "customer02@demo.local", "Demo Address 02", "customer_demo_02", c2_hash))
            ...
            if cursor.fetchone()["count"] == 0:
                # Customer 1 (Demo Customer 01) rented Honda Civic (id 2)
