...
from utils.helpers import format_currency, format_date_display, export_to_csv, open_file_externally
...
class ReportsView(tk.Frame):
    ...
    # ---------------- CSV Exports ----------------
    def _finish_export(self, path: str, label: str):
        """Notifies the user and attempts to auto-open the exported CSV file."""
        opened = open_file_externally(path)
        if opened:
            messagebox.showinfo("Export Successful", f"{label} exported successfully.\nOpening file: {path}")
        else:
            messagebox.showinfo(
                "Export Successful",
                f"{label} exported to:\n{path}\n\n(Could not auto-open the file — no default app is associated with .csv files on this system.)"
            )
    ...
    def _export_cars_csv(self):
        ...
        self._finish_export(path, "Cars report")
    ...
    def _export_customers_csv(self):
        ...
        self._finish_export(path, "Customers report")
    ...
    def _export_rentals_csv(self):
        ...
        self._finish_export(path, "Rentals report")
    ...
    def _export_payments_csv(self):
        ...
        self._finish_export(path, "Payments report")
    ...
    def _export_revenue_csv(self):
        ...
        self._finish_export(path, "Revenue report")
