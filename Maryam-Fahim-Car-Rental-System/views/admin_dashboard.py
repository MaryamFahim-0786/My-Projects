...
from config import OWNER_NAME, THEME, FONTS, APP_TITLE
...
from views.styles import StatCard, create_button, create_scrollable_treeview, center_window, configure_status_tags, draw_bar_chart
...
class AdminDashboard(tk.Frame):
    ...
    def _build_layout(self):
        # ----------------- Left Sidebar -----------------
        self.sidebar = tk.Frame(self, bg=THEME["sidebar_bg"], width=250)
        ...
        tk.Frame(self.sidebar, bg=THEME["sidebar_active"], height=2).pack(fill="x", padx=18, pady=(0, 14))

        tk.Label(brand_frame, text="✦ MARYAM FAHIM", font=FONTS["title_medium"], fg=THEME["sidebar_text"], bg=THEME["sidebar_bg"]).pack(anchor="w")
        tk.Label(brand_frame, text="Fleet Management Studio", font=FONTS["small"], fg=THEME["primary_light"], bg=THEME["sidebar_bg"]).pack(anchor="w")
    ...
    def _show_overview(self):
        ...
        configure_status_tags(recent_tree)
        ...
        # Analytics: Fleet Availability + Revenue Trend (native Canvas charts, no extra deps)
        analytics_row = tk.Frame(overview_frame, bg=THEME["main_bg"])
        analytics_row.pack(fill="x", padx=16, pady=(0, 12))

        fleet_card = tk.Frame(analytics_row, bg=THEME["card_bg"], highlightbackground=THEME["card_border"], highlightthickness=1, padx=16, pady=14)
        fleet_card.pack(side="left", fill="both", expand=True, padx=(0, 6))
        tk.Label(fleet_card, text="Fleet Availability", font=FONTS["title_small"], fg=THEME["text_primary"], bg=THEME["card_bg"]).pack(anchor="w", pady=(0, 8))
        draw_bar_chart(
            fleet_card,
            data=[
                ("Available", car_stats["Available"], THEME["success"]),
                ("Rented", car_stats["Rented"], THEME["purple"]),
                ("Maintenance", car_stats["Maintenance"], THEME["warning"]),
            ],
            width=380,
            height=170,
        )

        revenue_card = tk.Frame(analytics_row, bg=THEME["card_bg"], highlightbackground=THEME["card_border"], highlightthickness=1, padx=16, pady=14)
        revenue_card.pack(side="right", fill="both", expand=True, padx=(6, 0))
        tk.Label(revenue_card, text="Revenue - Last 6 Months", font=FONTS["title_small"], fg=THEME["text_primary"], bg=THEME["card_bg"]).pack(anchor="w", pady=(0, 8))
        monthly = Payment.get_monthly_revenue(months=6)
        draw_bar_chart(
            revenue_card,
            data=[(m["label"], m["total"], THEME["primary"]) for m in monthly],
            width=380,
            height=170,
            value_formatter=lambda v: format_currency(v, short=True),
        )
