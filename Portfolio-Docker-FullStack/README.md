# Maryam Fahim - AI Portfolio

> An AI-powered portfolio for **Maryam Fahim**, AI Full-Stack Developer. Built on the open-source [AI Portfolio template by Roy Amit](https://github.com/royamit1/Portfolio) (MIT License), customised with my own knowledge base, chatbot persona, projects and branding.
...
```bash
git clone https://github.com/MaryamFahim-0786/My-Projects.git
cd My-Projects
...
**Maryam Fahim** - AI Full-Stack Developer

- GitHub: [@MaryamFahim-0786](https://github.com/MaryamFahim-0786)
- LinkedIn: [Maryam Fahim](https://www.linkedin.com/in/maryam-fahim-9b2111339)

## 🙏 Credits

The original template (architecture, hybrid-search RAG pipeline, streaming chat UI) was created by **Roy Amit** and is used under the MIT License. See [LICENSE](LICENSE).

## 🧩 Personalising this project (where things live)

| What | Where |
|---|---|
| Chatbot knowledge | `backend/app/data/*.txt` and `backend/app/data/projects/*.txt` |
| Chatbot persona / rules | `backend/app/services/agent_service.py` |
| Name, title, social links | `frontend/lib/config.ts` |
| Project cards + images | `frontend/features/projects/data/projects.tsx`, `frontend/public/images/projects/` |
| Skills | `frontend/features/skills/data/skills.ts` |
| About Me text | `frontend/features/chat/components/templates/about-me-template.tsx` |
| Profile photo | `frontend/public/profile.webp` |
| Resume | `frontend/public/resume.pdf` |

Editing any file in `backend/app/data/` automatically triggers a fresh re-index the next time the backend starts.

## 🤖 Choosing the AI provider (free or paid)

The chatbot works with either provider. Set `LLM_PROVIDER` in `backend/.env`:

| Provider | Setting | Cost |
|---|---|---|
| Google Gemini | `LLM_PROVIDER="gemini"` + `GOOGLE_API_KEY` (from https://aistudio.google.com/apikey) | free tier |
| OpenAI | `LLM_PROVIDER="openai"` + `OPENAI_API_KEY` | prepaid credit |

Switching provider automatically creates a fresh vector index (embedding sizes differ).
...
# edit backend/.env: set LLM_PROVIDER + its API key, RESEND_API_KEY, OWNER_EMAIL
