...
interface ContactCard {
    ...
    description: string
    color: string
    action: "linkedin" | "github" | "whatsapp" | "contact" | "resume"
}
...
const CONTACT_CARDS: ContactCard[] = [
    ...
    },
    {
        ...
        description: "Quick chat, no sign-up needed",
        color: "from-green-500/20 to-emerald-500/20 hover:from-green-500/30 hover:to-emerald-500/30 border-green-500/20",
        action: "whatsapp",
    },
    {
        icon: FaEnvelope,
                        ...
                        >
                            {CONTACT_CARDS.map((card, i) => (
                                <motion.button
                                    ...
                                    className={`${i === CONTACT_CARDS.length - 1 && CONTACT_CARDS.length % 2 === 1 ? "sm:col-span-2" : ""} group flex items-center gap-4 p-4 rounded-xl border bg-gradient-to-br ${card.color} backdrop-blur-sm cursor-pointer transition-all duration-300 hover:scale-[1.03] hover:shadow-lg hover:shadow-black/20 active:scale-[0.98] text-left`}
                                >
