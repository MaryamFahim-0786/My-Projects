...
class DatabaseManager:
    ...
    def seed_demo_data(self):
        ...
        with self.transaction() as cursor:
            ...
            # Maryam Fahim demo customer (added if missing)
            cursor.execute("SELECT id FROM customers WHERE lower(username) = ?", ("maryam_fahim",))
            maryam_row = cursor.fetchone()
            if not maryam_row:
                m_hash, m_salt = hash_password("Maryam@123")
                cursor.execute("""
                    INSERT INTO customers (full_name, cnic, phone, email, address, date_of_birth, username, password_hash, salt, status, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    "Maryam Fahim", "00000-0000000-0", "03000000000",
                    "maryam.fahim@example.com", "Islamabad, Pakistan", "2000-01-01",
                    "maryam_fahim", m_hash, m_salt, CUSTOMER_STATUS_ACTIVE, now
                ))
                maryam_id = cursor.lastrowid
                cursor.execute("""
                    INSERT INTO accounts (account_number, customer_id, account_type, balance, status, created_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, ("1000000003", maryam_id, ACCOUNT_TYPE_SAVINGS, "100000.00", ACCOUNT_STATUS_ACTIVE, now))
                m_acc_id = cursor.lastrowid
                cursor.execute("""
                    INSERT INTO transactions (transaction_id, account_id, transaction_type, amount, balance_before, balance_after, description, related_account, transaction_date, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    "TXN202608270003", m_acc_id, TRANSACTION_TYPE_DEPOSIT, "100000.00", "0.00", "100000.00",
                    "Maryam Fahim Demo Account Opening Deposit", None, now, TRANSACTION_STATUS_COMPLETED
                ))
                log_event("DEMO_DATA", "Maryam Fahim demo customer and account created.")
