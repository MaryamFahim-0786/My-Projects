...
class CustomerService:
    ...
    def delete_customer(self, customer_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            with self.db.transaction() as tx_cursor:
                # Hard-delete the customer's dependent transaction history first,
                # then settled accounts, then the customer record. This keeps
                # SQLite foreign-key constraints satisfied while supporting the
                # admin CRUD delete operation.
                tx_cursor.execute("""
